        ...
        <section className="mx-auto grid min-h-[calc(100vh-12rem)] max-w-[var(--editable-container)] gap-6 px-4 py-12 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:px-8">
          <div className="astra-gradient relative flex flex-col justify-center overflow-hidden rounded-[1.7rem] p-8 text-white shadow-[0_24px_80px_rgba(61,24,112,0.16)] sm:p-12 lg:p-16">
            ...
            <div className="relative">
              ...
              <h1 className="mt-5 max-w-xl text-5xl font-black leading-tight tracking-[-0.045em] sm:text-7xl">{pagesContent.auth.login.title}</h1>
              <p className="mt-6 max-w-lg text-sm font-medium leading-8 text-white/78">{pagesContent.auth.login.description}</p>
            </div>
          </div>
          <div className="flex flex-col justify-center rounded-[1.7rem] bg-white p-7 shadow-[0_24px_80px_rgba(61,24,112,0.10)] sm:p-12 lg:p-16">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[#6d00df]">Member access</p>
            <h2 className="mt-3 text-4xl font-black tracking-[-.04em]">{pagesContent.auth.login.formTitle}</h2>
            <EditableLocalLoginForm />
            <p className="mt-5 border-t border-[#ded4f1] pt-5 text-sm text-[#465166]">New here? <Link href="/signup" className="font-black text-[#6d00df] underline-offset-4 hover:underline">{pagesContent.auth.login.createCta}</Link></p>
          </div>
        </section>
      </main>
    </EditableSiteShell>
