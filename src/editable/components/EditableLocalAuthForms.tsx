...
const inputClass = 'h-[3.25rem] rounded-full border border-[#ded4f1] bg-[#fbfbff] px-5 text-base font-bold text-[#08172f] outline-none transition placeholder:text-[#778095] focus:border-[#6d00df]'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-full bg-[#6d00df] px-6 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:bg-[#100033] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1rem] border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
